    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            inline half3 CalSpecular(half3 baseColor, half3 normal, half3 viewDir, half3 lightDir, half3 lightColor)
            {
                ...
                float spec = smoothstep(0.005f, 0.1f, pow(clamp(dot(reflectionVector, viewDir), 0.0f, 1.0f), _SpecPow));
            ...
            half4 frag (Varyings i) : SV_Target
            {
                ...
                float3 finalcolor = color.rgb * rcol.rgb;

                finalcolor = lerp(_RimColor.rgb, finalcolor, pow(abs(dot(normalWS, viewDirectionWS)), _RimPow));
